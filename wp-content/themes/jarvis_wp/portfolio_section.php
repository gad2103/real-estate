 ...
 <?php      global $root, $post_id;
	  ...
	  $portfolio_filters = get_terms('listings_filter');
...
<div id="portfolio-wrap">
	...
	//	$temp = $wp_query;
       ...
       global $wp_query;
        ...
        <?php $terms = get_the_terms( get_the_ID(), 'listings_filter' ); ?>      
                    
        <div class="<?php if($terms) : foreach ($terms as $term) { echo 'term-'.$term->slug.' '; } endif; ?>portfolio-item <?php echo $portfolio_grid; ?>">
            
            <?php 
            /* TODO FIX THE LISTINGS OVERLAY HERE */
                   $taxonomy = strip_tags( get_the_term_list($post->ID, 'listings_filter', '', ', ', '') );
					...
					if( get_post_meta( get_the_ID(), 'rnr_project_video_embed', true ) != "") {							
					 ...
					 if($smof_data['rnr_disable_portfolio_ajax']==true) {
					     ...
					     $link = '<a href="' . home_url() . '/#!'.__("listing-item","portfolio").'/'.$post->post_name.'" title="'. get_the_title() .'" class="portfolio-image">';
					 }
